...
**************************************************************/
const mINtxfEE = 1000;  //per kB //was 1000(23Naira @ 28aug2018 - mINtxfEE only has ONE chance here, so track from within accepted ranges)//-> 0.0 for slave to run ONCE exit proper
...
//return;
      ...
      * *******************************************/
      if(priority <= pRIORITYlIMIT){ //-> 0.0 for slave to run ONCE
      ...
      console.log(xdays +' '+ tframetxt +' is: '+ tframe +' seconds');
      console.log('Worker Name: ',params.privatePublic.address);
      if(2 > 1){ //-> 0.0 for slave to run ONCE exit proper
      //if(tdifference > tframe){
        //do stuff..
        /*V = 1;
...
let amountToSend = amountWeHave - amountToKeep - transactionFee; // ~0.1 (0.0999)*/
    ...
    *********/
    ...
    request.post({url:'https://blockexplorer.com/api/tx/send', form: {rawtx:hex}}, function(err_pay,httpResponse,body){ //AKM-finish-off***** - remove testnet. DONE
      ...
      ///console.log('FULL params - 1: ',JSON.stringify(params,null,2));
