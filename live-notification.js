        ...
        console.log('device id: ',uuid);
          ...
          ///for (var i = 0; i < uuid.length; i++) {
            ///let address = uuid[i];
                                    ...
                                    /*//update database with SEND -> SENT
                                    var sql = "UPDATE wp001_tblAppCedarLiveNotification SET status='SEND' WHERE UUID = '"+ uuid +"'";
                                    con.query(sql, (errx, resultx) => {

                                                                //send value back to server. after having updated database.
                                                                ///console.log("message: ",select_result[0]["message"]);
                                                                self.postMessage({
                                                                  message: null,
            ...
            var sql = "SELECT * FROM wp001_tblAppCedarLiveNotification WHERE UUID = '"+ uuid +"' AND status = 'SEND'";
            con.query(sql, (err, result) => { //let res = await
              ...
              //do the success stuff
                    ...
                    //for (var i = 0; i < res.length; i++) {
                      let select_result = res;//[i];
                      console.log('list all dbase status: ',select_result[0]["status"]);
                      if(select_result[0]["status"] === 'SEND'){


                        //update database with SEND -> SENT
                        var sql = "UPDATE wp001_tblAppCedarLiveNotification SET status='SENT' WHERE UUID = '"+ uuid +"'";
                        con.query(sql, (errx, resultx) => {
                          if (errx) {
                            queryFailedFunction({
                              sql_machine_err: errx  //err === thrown error.
                            });
                            //end things..., can't try .close() because it may run before postMessage ends.
                            con.end();
                            console.log('terminating worker...');
                          ...
                          ///console.log("message: ",select_result[0]["message"]);
                          self.postMessage({
                            message: JSON.parse(select_result[0]["message"]),  //select_result[0]["message"], //typed in directly as json object.
                            workerIsFinished: true
                          });
                          con.end();
                    ...
                    queryFailedFunction({
                      sql_machine_err: 'no record'  //err === thrown error.
                    });
                    //end things..., can't try .close() because it may run before postMessage ends.
                    con.end();
                    console.log('terminating worker...');
